...
FMOD_OPENSTATE _openState1 = FMOD_OPENSTATE_READY;
FMOD_OPENSTATE _openState2 = FMOD_OPENSTATE_READY;
std::string _stateString = "";
unsigned int _percentage = 0;
int _numberTags = 0;
int _numberTagsUpdated = 0;
FMOD::Sound* soundDSP = nullptr;
FMOD::Sound* soundDSP1 = nullptr;
FMOD::Sound* soundDSP2 = nullptr;
FMOD::Channel* channelDSP = nullptr;
...
void incrementAndCheck() {
	...
	if (!_isPlaying) {
		...
		if (count >= 3) {
			...
			channelDSP->stop();
			_result = _system->playSound(soundDSP2, _channelGroupOption, false, &channelDSP);
			channelDSP->setVolume(0.09);
			if (_result != FMOD_OK) {
				fprintf(stderr, "unable to play sound");
			}
			//_bgm endbgm
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int mode) {
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
		...
		if (!_isGameEntered) {
			...
			//channelDSP->stop();
			channelDSP->stop();
			_result = _system->playSound(soundDSP1, _channelGroupVoice, false, &channelDSP);
			channelDSP->setVolume(0.09);
			if (_result != FMOD_OK) {
				fprintf(stderr, "unable to play sound");
			}
...
int main(int args, char* argv) {
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		if (initialBGM) {
			...
			if (_result != FMOD_OK) {
				fprintf(stderr, "Unable to initialize FMOD system1");
				return false;
			}
			_result = soundDSP->getOpenState(&_openState, &_percentage, &_isStarving, 0);
			if (_result != FMOD_OK) {
				fprintf(stderr, "Unable to initialize FMOD system2");
				return false;
			}
			ss.str("");
			ss1.str("");
			...
			_result = soundDSP->getNumTags(&_numberTags, &_numberTagsUpdated);
			//get tags information
			for (unsigned int i = 0; i < _numberTags; i++) {
				FMOD_TAG tag;
				_result = soundDSP->getTag(0, i, &tag);
				if (_result != FMOD_OK) {
					fprintf(stderr, "Unable to initialize FMOD system3");
					return false;
				}

				if (tag.type == FMOD_TAGTYPE_FMOD) {
					if (!strcmp(tag.name, "Sample Rate Change") && channelDSP) {
						float frequency = *((float*)tag.data);
						_result = channelDSP->setFrequency(frequency);
						if (FMOD_OK != _result) {
							fprintf(stderr, "Unable to initialize FMOD system4");
						}
					}
				}
				else if (tag.datatype == FMOD_TAGDATATYPE_STRING) {
					char buffer[255];
					std::string check = tag.name;
					if ((check =="icy-br") || ((check == "icy-pub")) ) {

					}
					else if (check == "icy-url") {
						sprintf_s(buffer, "%s = '%s' (%d bytes)", "URL", (char*)tag.data, tag.datalen);
						_text->addLine(buffer, ForProj4++);
					}
					else if (check == "icy-genre") {
						sprintf_s(buffer, "%s = '%s' (%d bytes)", "GENRE ", (char*)tag.data, tag.datalen);
						_text->addLine(buffer, ForProj4++);
					}
					else if (check == "icy-name") {
						sprintf_s(buffer, "%s = '%s' (%d bytes)", "NAME", (char*)tag.data, tag.datalen);
						_text->addLine(buffer, ForProj4++);
					}
					else {
			...
			ForProj4 = 18;
			_numberTags = 0;
			_numberTagsUpdated = 0;
			_result = _system->update();
			if (_result != FMOD_OK) {
				fprintf(stderr, "Unable to initialize FMOD system7");
				return false;
			}
			_result = soundDSP1->getOpenState(&_openState1, &_percentage, &_isStarving, 0);
			if (_result != FMOD_OK) {
				fprintf(stderr, "Unable to initialize FMOD system8");
				return false;
			}
			ss.str("");
			ss1.str("");
			...
			ss1.str("");
			/*ss = _percentage;
			_text->addLine(_percentage, ForProj4--);

			_text->addLine(_isStarving, ForProj4)*/
			_result = soundDSP1->getNumTags(&_numberTags, &_numberTagsUpdated);
			//get tags information
			for (unsigned int i = 0; i < _numberTags; i++) {
				FMOD_TAG tag;
				_result = soundDSP1->getTag(0, i, &tag);
				if (_result != FMOD_OK) {
					fprintf(stderr, "Unable to initialize FMOD system9");
					return false;
				}
			...
			_result = soundDSP2->getOpenState(&_openState2, &_percentage, &_isStarving, 0);
			if (_result != FMOD_OK) {
				fprintf(stderr, "Unable to initialize FMOD system");
				return false;
			}
			...
			ss.str("");
			ss1.str("");
			_result = soundDSP2->getNumTags(&_numberTags, &_numberTagsUpdated);
			//get tags information
			for (unsigned int i = 0; i < _numberTags; i++) {
				FMOD_TAG tag;
				_result = soundDSP2->getTag(0, i, &tag);
				if (_result != FMOD_OK) {
					fprintf(stderr, "Unable to initialize FMOD system");
					return false;
				}
		...
		if (channelDSP) {
			_result = channelDSP->getPaused(&_isPaused);
			if (FMOD_OK != _result) {
				fprintf(stderr, "Unable to get is paused for channel at index 0\n");
			}
			_result = channelDSP->isPlaying(&_isPlaying);
			if (FMOD_OK != _result) {
				fprintf(stderr, "Unable to get is playing for channel at index 0\n");
			}

			_result = channelDSP->setMute(_isStarving);
			if (FMOD_OK != _result) {
				fprintf(stderr, "Unable to mute channel at index 0\n");
			}
...
bool initFMOD() {
	...
	_result = _system->createSound(vecURLS.at(1).c_str(), FMOD_CREATESTREAM | FMOD_NONBLOCKING, 0, &soundDSP1);
	if (_result != FMOD_OK) {
		fprintf(stderr, "Unable to set file buffer size");
		return false;
	}

	_result = _system->createSound(vecURLS.at(2).c_str(), FMOD_CREATESTREAM | FMOD_NONBLOCKING, 0, &soundDSP2);
	if (_result != FMOD_OK) {
		fprintf(stderr, "Unable to set file buffer size");
		return false;
	}
	...
	//Create groups
	_result = _system->createChannelGroup("Initial BGM", &_channelGroupBGM);
	if (_result != FMOD_OK) {
		fprintf(stderr, "Unable to create channel group");
		return false;
	}

	_result = _system->createChannelGroup("Middle BGM", &_channelGroupVoice);
	if (_result != FMOD_OK) {
		fprintf(stderr, "Unable to create channel group");
		return false;
	}

	_result = _system->createChannelGroup("Final BGM", &_channelGroupOption);
	if (_result != FMOD_OK) {
		fprintf(stderr, "Unable to create channel group");
		return false;
	}
...
void shutdown() {
	...
	while (_openState != FMOD_OPENSTATE_READY) {
		...
		_result = soundDSP->getOpenState(&_openState, 0, 0, 0);
		if (_result != FMOD_OK) {
			fprintf(stderr, "Unable to close system");
		}
	}
	while (_openState1 != FMOD_OPENSTATE_READY) {
		_result = _system->update();
		if (_result != FMOD_OK) {
			fprintf(stderr, "Unable to close system");
		}

		_result = soundDSP1->getOpenState(&_openState1, 0, 0, 0);
		if (_result != FMOD_OK) {
			fprintf(stderr, "Unable to close system");
		}
	}
	while (_openState2 != FMOD_OPENSTATE_READY) {
		_result = _system->update();
		if (_result != FMOD_OK) {
			fprintf(stderr, "Unable to close system");
		}

		_result = soundDSP2->getOpenState(&_openState2, 0, 0, 0);
		if (_result != FMOD_OK) {
			fprintf(stderr, "Unable to close system");
		}
	}
